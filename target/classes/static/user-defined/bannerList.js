...
function bannerListFn(a,b,e,f,w){
    var $bannerMaxWapDom=a;
    // var windowWidth=$(window).width();  //全屏
    var windowWidth=w; //图片宽度
    var timeShow=0;
    var array=0;
    var timeOff=0;

    var imgPos=$bannerMaxWapDom.find("ul").find("li");  //滚动的内容
    ...
    $bannerMaxWapDom.find("ul").width(liWidth*(liLength+1));//全部长度
    ...
    function bannerImgList(a){
        $bannerMaxWapDom.find("ul").animate({marginLeft: -a*windowWidth},400)
    }
    ...
    function move(){
        ...
        if (array == liLength){
            $bannerMaxWapDom.find("ul").css({marginLeft:0});
            array=1;
        }

        if (array == -1){
            $bannerMaxWapDom.find("ul").css({marginLeft:-liWidth*(liLength-1)});
            array=liLength-2
        }

        $bannerMaxWapDom.find("ul").stop(true).animate({
            marginLeft:-array*liWidth
        });
